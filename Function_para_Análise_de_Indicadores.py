...
#Percentual = (Vendas Perdidas por Estoque) / (Vendas Concluídas + Vendas Perdidas por Estoque)


# definir função se stockout
#Percentual = (Vendas_Perdidas_Estoque) / (Vendas_Concluídas + Vendas_Perdidas_Estoque)

def Stockout(d_vendas):
    Vendas_Perdidas_Estoque = 0
    Vendas_Concluídas = 0
    for item in d_vendas:
        valor, status, motivos = d_vendas[item]
        if status == 'Concluído':
            Vendas_Concluídas += valor
        elif status == 'Cancelado' and motivos == 'Estoque em Falta':
            Vendas_Perdidas_Estoque += valor
            Vendas_Concluídas += valor
    return Vendas_Perdidas_Estoque / Vendas_Concluídas
...
#sua function aqui
def inadiplencia(lista_devedores):
    for item in lista_devedores:
        inadiplentes = []
        cpf, valor, dias = item
        if valor > 1000 and qtde > 20:
            inadiplentes.append(cpf)
    return inadiplentes 
...
print(inadiplencia(clientes_devedores))
